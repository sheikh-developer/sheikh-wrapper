      ...
      <div className="mb-6 overflow-x-auto rounded-lg bg-muted p-4">
        <pre>
          <code className="language-mermaid">
            {`%% --- %%
title: LLM Chat App Flow (Scaffolded & Collapsible)
%% ---
graph TD
    %% Scaffolding into Client and Server sections for clarity and collapse functionality.
    %% The Client-Side is where mobile-first responsive design is handled.
    subgraph Client-Side (Responsive & Mobile-First UI)
        A[User types message] --> B[Submits message (Enter/click)]
        B --> C[Display Optimistic UI Update]
        H[Render incoming data stream] --> I[Finalize Assistant Message in UI]
        M[Display Tool Output as Message] --> L[Idle / Await Next Input]
        I --> L
    end

    subgraph Server-Side (Backend API & Logic)
        D[API Endpoint Receives Request] --> E{Stream response?}
        E -- Yes --> F[Stream tokens via SSE/WebSocket]
        E -- No --> G[Buffer full response]
        J{Response requires a Tool?}
        F --> J
        G --> J
        J -- Yes --> K[Execute Tool (e.g., Search, Code Gen)]
    end
